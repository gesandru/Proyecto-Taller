#import "../include/const.h"
#import "../include/VarEqn.h"

Matrix static varEqn(double x, Matrix yPhi){

//Mirar si hay que cambiar las llamadas a estos en la funcion
Const::AuxParam;
Const::eopdata;
...
//esto hay que cambiarlo cuando cambie IERS
...
Matrix P = PrecMatrix::precMatrix(Const::MJD_J2000,AuxParam.Mjd_TT + x/86400.0);
Matrix N = NutMatrix::nutMatrix(AuxParam.Mjd_TT + x/86400.0);
Matrix T = N * P;
Matrix E = PoleMatrix::poleMatrix(x_pole,y_pole) * GHAMatrix::ghaMatrix(Mjd_UT1) * T;
...
for(int j = 1; j<=6;j++){
    //esto falta de cambiar
    Phi(:,j) = yPhi(6*j+1:6*j+6);
}

// Acceleration and gradient
Matrix a = AccelHarmonic::accelHarmonic( r, E, AuxParam.n, AuxParam.m );
Matrix G = G_AccelHarmonic::g_AccelHarmonic( r, E, AuxParam.n, AuxParam.m );

// Time derivative of state transition matrix

//esto no sé si dejarlo como matriz o cómo vector
Matrix yPhip = Matrix(42);
